...
class HttpApiTests(@Autowired val mockMvc: MockMvc) {
    ...
    @Test
    fun `List articles`() {
        val jazzinjars = User("springjazz", "Jazz", "InJars")
        val spring5Article = Article("Spring Framework 5.0 goes GA", "Dear Spring community ...", "Lorem ipsum", jazzinjars, 1)
        val spring43Article = Article("Spring Framework 4.3 goes GA", "Dear Spring community ...", "Lorem ipsum", jazzinjars, 2)
        ...
        mockMvc.perform(get("/api/article/").accept(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("\$.[0].author.login").value(jazzinjars.login))
                .andExpect(jsonPath("\$.[0].id").value(spring5Article.id!!))
                .andExpect(jsonPath("\$.[1].author.login").value(jazzinjars.login))
    ...
    fun `List users`() {
        ...
        whenever(userRepository.findAll()).thenReturn(listOf(jazzinjars, smaldini))
        mockMvc.perform(get("/api/user/").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk)
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("\$.[0].login").value(jazzinjars.login))
